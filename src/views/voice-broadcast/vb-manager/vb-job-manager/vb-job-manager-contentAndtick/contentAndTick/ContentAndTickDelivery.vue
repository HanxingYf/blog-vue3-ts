  ...
  >
    ...
    <div class="content">
      ...
      <div class="content-right">
        <div class="content-right-title">分发</div>
        <NFilter
          :configs="formObj"
...
import { deliveryContentAndTask, getDeliveryContentAndTickDetail, recallContentAndDelivery } from '@/presenter/manager.pre'
...
import { DeliveryDetailListNumsI, DeliveryDetailListPersonI, DeliveryDetailListVertifyPersonI, ManagerContentAndTickTaskListRowI } from './type';

@Component({ components: { NFilter } })
export default class ContentAndTickDelivery extends Vue {
  @Prop({ type: Boolean, default: false }) readonly visible!: boolean
  @Prop({ type: Object, default: () => null }) readonly curDataRow!: ManagerContentAndTickTaskListRowI | null
  // 分发表单
  formObj: { [propName: string]: FilterConfigI } = {
    pa_name: {
      ...
      value: [],
      options: [],
      validate: () => this.formObj.pa_name.value.length ? '' : '请选择评测人员'
    },
    num: {
      type: 'input-number',
      label: '数量',
  ...
  verifyPersonDataSource: DeliveryDetailListVertifyPersonI[] = []
  ...
  @Watch('curDataRow', { immediate: true })
  onCurDataRowChange() {
    this.getContentAndTiclkDeliveryRecord()
  }
  ...
  async getContentAndTiclkDeliveryRecord() {
    if (!this.curDataRow) {
      return
    }
    const { id } = this.curDataRow
    this.tableLoading = true
    const res = await getDeliveryContentAndTickDetail(id)
    ...
    const {
      ...
      mark_person, verify_person, verified_num, Block_disp_num, block_num
    } = res
    // console.log(verify_person, '成功');
    ...
    this.verifyPersonDataSource = verify_person.map((v) => {
      return {
        dis_id: v.dis_id,
        type: v.type,
        job_name: v.person,
        dispatch_count: `${verified_num}/  ${verify_dispatch_num}`,
        disabled: v.complete_count === v.total_count
      }
    })
  }
  ...
  async handleOnRecall(row: DeliveryDetailListPersonI) {
    ...
    const { recalled, msg } = await recallContentAndDelivery(id, { dis_id, type });
  ...
  async onDeliveryContentTask() {
    ...
    const { pa_name, num, type } = this.formObj
    const { id } = this.curDataRow
    this.loading = true
    const { deliveried, msg } = await deliveryContentAndTask(`${id}`, {
  ...
  async mounted() {
    ...
    // const res = await getDeliveryContentAndTickDetail(389);
    // console.log(res, '+++');
  }
