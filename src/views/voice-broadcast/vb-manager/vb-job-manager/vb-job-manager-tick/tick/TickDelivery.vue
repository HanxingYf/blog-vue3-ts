...
import { ManagerTickTaskListRowI, DeliveryDetailListNumsI, DeliveryDetailListPersonI, } from './types';
...
export default class TickDelivery extends Vue {
  @Prop({ type: Boolean, default: false }) readonly visible!: boolean
  @Prop({ type: Object, default: () => null }) readonly curDataRow!: ManagerTickTaskListRowI | null
  ...
  onCurDataRowChange() {
    ...
    if (this.curDataRow) {
      this.formObj.num.maxNum = this.curDataRow.case_count
    }
  }
  ...
  async getDeliveryRecord() {
    ...
    const res = await getDeliveryDetail(id, 'tick')
  ...
  async handleOnRecall(row: DeliveryDetailListPersonI) {
    ...
    const { recalled, msg } = await recallDelivery(id, 'tick', { dis_id, type });
  ...
  async onDeliveryContentTask() {
    ...
    const { id } = this.curDataRow
    this.loading = true
    const { deliveried, msg } = await deliveryTask(`${id}`, 'tick', {
