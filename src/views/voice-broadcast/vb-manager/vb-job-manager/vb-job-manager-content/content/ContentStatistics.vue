      ...
      >
        <template slot-scope="{ row, index }" slot="contrast">
          <div
            ...
            @click="
              jumpTo(
                `/voice-broadcast/task-manager/content-task-detail?job_id=${job_id}&job_name=${job_name}&tag=${row.type}&path_consis=0`,
                row.contrast
              )
            "
          >
        ...
        <template slot-scope="{ row, index }" slot="questionType">
          <div
            ...
            @click="
              jumpTo(
                `/voice-broadcast/task-manager/content-task-detail?job_id=${job_id}&job_name=${job_name}&tag=${
                  row.sType || row.type
                }&issue_type=${row.questionType || ''}&path_consis=0`
              )
            "
          >
...
@Component
export default class ContentStatistics extends Vue {
  list: any[] = []
