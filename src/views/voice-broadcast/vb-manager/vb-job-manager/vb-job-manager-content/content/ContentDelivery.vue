...
import { deliveryTask, getDeliveryDetail, recallDelivery } from '@/presenter/manager.pre'
...
import { ManagerContentTaskListRowI, DeliveryDetailListNumsI, DeliveryDetailListPersonI, } from './types';
...
export default class ContentDelivery extends Vue {
  @Prop({type: Boolean, default: false}) readonly visible!: boolean
  @Prop({type: Object, default: () => null}) readonly curDataRow!: ManagerContentTaskListRowI | null
  ...
  formObj: { [propName: string]: FilterConfigI } = {
    ...
    blind: {
      ...
      validate: () => this.formObj.type.value ? '' : '请选择是否盲评'
    }
  }
  ...
  onTypeChange(val: number) {
    if (val == 1) { // 评测
      this.formObj.blind.hide = false
      this.formObj.tag.hide = true
    } else {
      this.formObj.blind.hide = true
      if (this.formObj.blind.value == 1) {  // 盲评
        this.formObj.tag.hide = true
      } else {
        this.formObj.tag.hide = false
      }
    }
  }
  ...
  async getDeliveryRecord() {
    ...
    const res = await getDeliveryDetail(id, 'content')
  ...
  async handleOnRecall(row: DeliveryDetailListPersonI) {
    ...
    const { recalled, msg } = await recallDelivery(id, 'content', { dis_id, type });
  ...
  async onDeliveryContentTask() {
    ...
    const { id } = this.curDataRow
    this.loading = true
    const { deliveried, msg } = await deliveryTask(`${id}`, 'content', {
      pa_name: pa_name.value,
      type: type.value,
      num: num.value,
      ...(tag.value.length ? { tag: tag.value } : {}),
      ...(blind.hide ? {} : { blind: !!blind.value })
    })
    this.loading = false
    if (deliveried) {
