...
import { deliveryTask, getDeliveryDetail, recallDelivery, ManagerContentOrTickOrCodeDiffNormalListI } from '@/presenter/manager.pre'
...
export default class CodeDiffDelivery extends Vue {
  @Prop({type: Boolean, default: false}) readonly visible!: boolean
  @Prop({type: Object, default: () => null}) readonly curDataRow!: ManagerContentOrTickOrCodeDiffNormalListI | null
  ...
  async getDeliveryRecord() {
    ...
    const res = await getDeliveryDetail(id, 'codediff')
  ...
  async handleOnRecall(row: DeliveryDetailListPersonI) {
    ...
    const { recalled, msg } = await recallDelivery(id, 'codediff', { dis_id, type });
  ...
  async onDeliveryTask() {
    ...
    const { id } = this.curDataRow
    this.loading = true
    const { deliveried, msg } = await deliveryTask(`${id}`, 'codediff', {
