...
export default class DeliveryModal extends Vue {
  ...
  @Prop({type: Object, default: () => null}) readonly curDataRow!: vectorManager.VectorJobListI | null
  ...
  @Watch('curDataRow')
  onCurDataRowChange() {
    this.formObj.pa_name.value = []
    this.formObj.num.value = 0
    this.formObj.type.value = 1
  ...
  async getDeliveryRecord() {
    ...
    const res = await vectorManager.getDeliveryDetail(id)
  ...
  async handleOnRecall(row: DeliveryDetailListPersonI) {
    ...
    const { recalled, msg } = await vectorManager.recallDelivery(id, { dis_id, type });
  ...
  async onDeliveryContentTask() {
    ...
    const { pa_name, num, type, comment } = this.formObj
    const { id, info } = this.curDataRow
    this.loading = true
    const { deliveried, msg } = await vectorManager.deliveryJob(`${id}`, {
