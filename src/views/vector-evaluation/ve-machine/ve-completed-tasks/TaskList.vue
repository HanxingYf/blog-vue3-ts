<template>
  <div class='task-list'>
        ...
        <template slot-scope="{ row, index }" slot="action">
          <div class="action-btn-group">
            ...
            <Button
              ...
              :to="`/vector-evaluation/completed-tasks/task-detail?task_id=${row.id}&task_name=${row.task_name}&task_num=${row.num}`">
              详情
            </Button>
    ...
    <DeliveryModal :visible="isModalOpen" :curDataRow="curDataRow" @on-close="onClose"/>
  </div>
</template>
...
export default class TaskList extends Vue {
  ...
  dataSource: vectorMachine.TaskListI[] = []
  total: number = 0
  curPageNum: number = 1
  curDataRow: vectorMachine.TaskListI | null = null
  ...
  async handleOnPullTaskList(page: number) {
    ...
    const res = await vectorMachine.getTasksByStatus('Completed', {
      page,
      per_page: 10,
      id: id.value,
  ...
  handleOnDistribute(row: vectorMachine.TaskListI) {
    this.curDataRow = row
    this.isModalOpen = true
  }
  onClose(shouldFreash: boolean) {
    this.isModalOpen = false
    shouldFreash && this.handleOnPullTaskList(this.curPageNum)
  }
  async handleOnDeleteTask(row: vectorMachine.TaskListI, index: number) {
    const { deleted, msg } = await vectorMachine.deleteTask(row.id)
