  ...
  <div class="task-manager-list">
    ...
    <div class="table-wrapper">
      ...
      <Table :columns="columns" :data="dataSource" border :loading="loading" size="small">
        ...
        <template slot-scope="{ row, index }" slot="num_ratio">
        ...
        <template slot-scope="{ row, index }" slot="action">
          <Button
            type="primary"
            size="small"
            :to="`/laneline-evaluation/task-evaluation/task-detail?job_id=${row.id}&job_name=${row.job_name}&dispatch_type=${row.dispatch_type}`">
            详情
          </Button>
        </template>
      </Table>
...
export default class ContentList extends Vue {
  ...
  dataSource: lanelineManager.LanelineEvaluationListI[] = []
  ...
  onJobTypeChanged(val: number) {
    ...
    window.localStorage.setItem('laneline_select_job_type', `${val}`)
  }
  ...
  async handleOnGetJobList(page: number) {
    ...
    const res = await lanelineManager.getPaJobList({
      page,
      per_page: 10,
  ...
  // hooks
  mounted() {
    const select_job_type = window.localStorage.getItem('laneline_select_job_type')
    if (select_job_type) {
