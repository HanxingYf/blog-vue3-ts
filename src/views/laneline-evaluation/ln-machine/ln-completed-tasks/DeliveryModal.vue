...
export default class DeliverModal extends Vue {
  ...
  @Prop({ type: Object, default: () => null }) readonly curDataRow!: lanelineMachine.TaskListI | null
  ...
  async handleOnDistributeJob() {
    ...
    const {
      job_name, city, to_user, diff_type, custom_filter,
      num, desc, dead_line
    } = this.formObj
    this.loading = true
    const { distributed, msg } = await lanelineMachine.distributeTask(id, {
      city: city.value,
  ...
  async mounted() {
    // 获取高级筛选规则
    const result = await getHighFilterRules('lane', {})
